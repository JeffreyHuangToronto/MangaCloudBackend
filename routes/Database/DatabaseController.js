        ...
        if (manga == null) {
                ...
                .then(async (response) => {
                    ...
                    $("div.chapter-list > div > span > a").each(
                        async (index, element) => {
                            ...
                            schema.chapters.push(
                                Number(
                                    $(element).attr("href").split("chapter-")[1]
                                )
                            );
                        }
                    );
            ...
            await axios
                ...
                .then(async (response) => {
                    ...
                    $("div.chapter-list > div > span > a").each(
                        async (index, element) => {
                            //   console.log($(element).attr("href").split("chapter-")[1]);
                            if (
                                isNaN(
                                    Number(
                                        $(element)
                                ...
                                console.log(
                                    "Found one that is not a number 1",
                                    $(element).attr("href")
                                );
                            }
                            schema.chapters.push(
...
async function saveAllManga(source) {
    ...
    if (source == MangaSources.MangaKakalot.Source_Name) {
        ...
        await axios
            .get(
                `https://mangakakalot.tv/manga_list/?type=newest&category=all&state=all&page=${page_number}`
            )
            .then(async (response) => {
                const $ = cheerio.load(response.data); // Load the response
                $("div.group_page > a.page_blue.page_last").each(
                    async (index, element) => {
                        MAX_PAGES = Number(
                            $(element)
                                .text()
        ...
        while (page_number <= MAX_PAGES) {
            await axios
                ...
                .then(async (response) => {
                    ...
                    // let total_chapters = [];
                    // Find the manga id
                    $("div.leftCol.listCol > div > div > h3 > a").each(
                        async (index, element) => {
                            // console.log($(element).text());
                            titles.push($(element).attr("title"));
                            manga_ids.push(
                                $(element).attr("href").replace("/manga/", "")
                            );
                        }
                    );
                    ...
                    $("div.leftCol.listCol > div > div > p").each(
                        async (index, element) => {
                            if (index != 0) {
...
async function saveAllCompletedManga(source) {
    ...
    if (source == MangaSources.MangaKakalot.Source_Name) {
        ...
        await axios
            .get(
                `https://mangakakalot.tv/manga_list/?type=newest&category=all&state=Completed&page=${page_number}`
            )
            .then(async (response) => {
                const $ = cheerio.load(response.data); // Load the response
                $("div.group_page > a.page_blue.page_last").each(
                    async (index, element) => {
                        ...
                        // console.log(
                        //   $(element).text().replace("Last(", "").replace(")", "")
                        // );
                    }
                );
            })
...
async function getCompletedManga(source, page) {
    ...
    const Db = client.db("Manga");
    const Collection = Db.collection(`MangaKakalot Completed`);
    if (page == null) {
        page = 0;
    }
