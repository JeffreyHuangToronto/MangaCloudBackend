...
router.get("/", async function (req, res, next) {
    ...
    await setChapter(req.query.userId, req.query.novelId, req.query.chapter);
    res.send("Added!");
    res.end();
});

async function setChapter(userId, novel_id, chapter) {
    console.log(chapter);
    if (userId == null || isNaN(userId)) {
        ...
        if (user == null) {
            ...
            const db = await client
                .db("NAMS")
                .collection("USERS")
                .updateOne({ _id: userId }, { $set: { novel_id: novel_id, currentChapter: chapter } });
            return user.library;
        }
    }
}
