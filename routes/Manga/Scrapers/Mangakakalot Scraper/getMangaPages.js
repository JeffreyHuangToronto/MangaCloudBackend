...
router.get("/", async function (req, res, next) {
  ...
  if (
    !(await DatabaseController.findMangaChapter(
      source,
      manga_id,
      chapter_index
    ))
    ...
    res.send(
      await DatabaseController.getMangaChapter(source, manga_id, chapter_index)
    );
  }
  res.end();
});
...
async function scrapeChapter(source, manga_id, chapter_index, res) {
  ...
  await axios
    .get(
      `https://mangakakalot.tv/chapter/${manga_id}/chapter-${r_chapter_number.toString()}`
    )
    .then(async (response) => {
      ...
      $(
        "body > div:nth-child(2) > div > div > p > span:nth-child(4) > a > span"
      ...
      if (
        schema.manga_title == "" ||
        !(schema.source in MangaSources) ||
        r_chapter_number < 0 ||
        schema.number_of_manga_pages == 0
      ) {
